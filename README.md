...
Primarily for personal use—signatures and repository structure may change at any time.
Feel free to use any code if you find it helpful, but mind that the code written at the beginning of my PhD
(February 2024), might be..., well, bad ¯\_(ツ)_/¯ 

For convenience, the files that *should* be usable are marked with *, while those that are very unlikely to work (and should be removed) with #. If no symbol is given, the code should work, but will likely not be compatibile with the rest of the repository.

### Repository Structure
Last updated on version: 0.2.8

```
novami/
├── deprecated/   Old code kept because it might be useful one day
├── environments/   Environments for special-need code (CDDD, Mordred) 
├── novami/   Main directory with code
│   ├── api/
...
│   ├── cli/
│   │   ├── CDDD.py*   CLI wrapper for calculating Mordred descriptors
...
│   │   ├── Mordred.py*   CLI wrapper for calculating Mordred descriptors
...
│   │   └── file.py*   IO functions for any (most) formats I'm using; works with Pandas/Polars DFs
│   ├── metrics/
│   │   └── modellability.py   MODI index
│   ├── ml/  Everything Machine-Learning related; I'm quite proud of this implementation :) 
...
├── projects/  More-or-less completed projects
...
│   └── qcg_template   Template for QCG PilotJob training on bigger scale 
